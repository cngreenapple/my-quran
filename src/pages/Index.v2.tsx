    ...
    <div className="min-h-dvh bg-background">
      ...
      >
          ...
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-600 via-emerald-700 to-teal-800 text-white p-4 sm:p-5 shadow-lg shadow-emerald-500/20">
            ...
            <div className="relative flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                ...
                <h1 id="hero-title" className="text-base sm:text-lg font-bold leading-tight mb-0.5">
                  Assalamu'alaikum
                </h1>
              ...
              <div className="text-right shrink-0">
                <p className="font-arabic text-xl sm:text-2xl text-emerald-50/90 leading-none" dir="rtl" lang="ar">
          ...
          <nav aria-label="Menu akses cepat">
            <ul className="grid grid-cols-4 sm:grid-cols-7 gap-2" role="list">
              {QUICK_ACTIONS.map((action) => {
                ...
                return (
                  <li key={action.to}>
                    <Link to={action.to} className="block group focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 rounded-xl" aria-label={action.label}>
                      <div className={cn("relative flex flex-col items-center justify-center gap-1.5 py-2.5 rounded-xl border border-border/60 bg-card hover:border-primary/30 transition-all active:scale-95", c.bg)}>
                        {badge && (
                        ...
                        <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center bg-gradient-to-br shadow-sm", c.grad)} aria-hidden="true">
        ...
        <section className="mb-4" aria-label="Asmaul Husna hari ini">
          <Link to="/asmaul-husna" className="block group focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 rounded-2xl">
            <div className="flex items-center gap-3 p-3.5 rounded-2xl border border-amber-500/25 bg-gradient-to-r from-amber-500/8 via-amber-500/4 to-transparent hover:border-amber-500/40 transition-all active:scale-[0.99]">
              <p className="font-arabic text-3xl sm:text-4xl text-foreground leading-none shrink-0" dir="rtl" lang="ar" aria-label={`${asmaOfTheDay.latin}: ${asmaOfTheDay.meaningId}`}>
                {asmaOfTheDay.arabic}
              </p>
        ...
        <section className="mb-3">
          ...
          <SearchBar value={query} onChange={setQuery} placeholder="Cari nama, arti, atau nomor..." />
        </section>
        ...
        <section aria-label="Daftar 114 surah Al-Qur'an">
          {isLoading ? (
            ...
            <ErrorState title="Gagal Memuat" message="Tidak dapat terhubung ke server. Periksa koneksi Anda dan coba lagi." onRetry={() => refetch()} />
